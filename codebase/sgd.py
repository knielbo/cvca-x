...
def main():
    ...
    for epoch in np.arange(0, args["epochs"]):
        ...
        for (batchX, batchY) in next_batch(trainX, trainY, args["batch_size"]):
            ...
            # dot product of current batch and weight matrix
        ...
        # uppdate loss history as average of batch loss
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("..","figures","sgd_data.png"))
    plt.close()
    ...
    plt.ylabel("$Loss$")
    plt.tight_layout()
    plt.savefig(os.path.join("..","figures","sgd_loss.png"))
    plt.close()
